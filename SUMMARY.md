...
- **Testing forum + nickname login (2026-07-22)**: /admin/forum — threads + replies for the testing crew; identity = nickname-only field added to the login page (cookie, no credentials while admin is open-access; email/password unchanged). Posts editable by their author (nickname match, "edited" marker); two seeded 📢 announcement threads explain the forum + testing phase (EN + 中文). Tables `forum_threads`/`forum_posts` in both backends. Owner also confirmed **Supabase (not self-hosted Postgres)** — see [docs/Database.md](docs/Database.md). **Tester guide**: /admin/guide (nav item "Guide") renders [docs/USER-GUIDE.md](docs/USER-GUIDE.md) (EN + 中文, owner-editable markdown).
