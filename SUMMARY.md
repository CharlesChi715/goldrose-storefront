...
- **Supabase activation IN PROGRESS (2026-07-22)**: hosted project created (region ap-southeast-2-ish, project ref cfvsvgbldnzkcjvbwnjp), migration run, clean seed done + announcements inserted, verified (catalog public / orders 401 / bucket ✓). Local `.env.local` now points at hosted. REMAINING: owner auth user + admin_users row → Vercel env vars + redeploy. Pending decision: ADMIN_OPEN_ACCESS override so nickname testers keep forum access on live.
