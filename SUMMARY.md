...
- **2026-07-26 interactions** (owner asks, all on the shared `components/home/Carousel.tsx`): Best Sellers, Shop by Occasion and Real Gifts/Real Moments rails **auto-slide one card at a time, slowly** (4.2s dwell / 0.9s glide; `cellWidth`+`step` advance one card inside a wider window; hero keeps its brisk timings). Best Sellers = the design's single card **+ 3 mocked copies** (4 dots) — the frame's smaller 2nd card is superseded until OQ-3. MORI path cards clickable (Find a Gift → `/shop`; Personalize → `#personalize`, Craft → `#craft`, i.e. the A-8/A-9 sections that already hold that content) and the 5 occasion chips → `/shop`. Product photos **hover-zoom to 1.06** inside their frames (shop grid, Best Sellers, A-3, product hero; standalone `scale` so it composes with the import's sub-pixel `translate`; pointer-only, reduced-motion opts out). `/shop` **paging works**: 1–5 → `?page=N`, arrow inert on the last page, each page rotates the same 8 placeholder cards into different slots (page 1 unchanged + bare URL; pages 2–5 noindex). `/account` type tabs cross-fade via `FadeLink`.
