 ...
 */
/**
 * THIS CARD IS READ-ONLY, AND THAT IS THE POINT (owner, 2026-08-08)
 *
 * Point-and-edit used to live here, which meant a transparent layer over the
 * frame to keep clicks off the storefront's real links. That layer swallowed
 * the wheel, so the scroll had to be re-issued programmatically — and the
 * storefront's own `scroll-behavior: smooth` turned each wheel event into an
 * eased animation that the next event cancelled. A gesture asking for 2,000px
 * moved 118. Scroll chaining at the ends and touch panning were lost with it.
 *
 * Pointing now happens on the section windows, whose film is inert, so none of
 * that machinery is needed anywhere and none of it is left. This frame answers
 * the pointer like any other page, which is why it scrolls like one.
 */
export function MainPreview({
  ...
  /** Handed up so the screen can write drafts into this frame too. */
