...
| `ixd-home-shop/` — `homepage.md`, `shop.md`, `assets/` (52 screenshots) | 2026-08-04 | **Figma** — interaction design is maintained there now, so these 2026-07-25 parse outputs (homepage `H-01…H-37`, shop `N-01…N-15`) stopped being the source of truth. The delivery they were parsed from is still live at `team-deliveries/originals/2026-07-25-home-shop-mechanism/` and remains the authority on wording. The `H-nn`/`N-nn` ids survive as bare ids in `components/home/` and `tests/e2e/homepage.spec.ts` — no path points back here. |
