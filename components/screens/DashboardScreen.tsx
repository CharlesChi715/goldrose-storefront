...
type Tile = { icon: string; ink: [number, number]; title: string; text: string; href?: string };
type Row = { title: string; value: string; href?: string };

function card(x: number, y: number, w: number, h: number, bg = CARD): React.CSSProperties {
  return { ...abs(x, y, w, h), background: bg, boxShadow: `inset 0 0 0 1px ${SAND}`, borderRadius: 14 };
}

function DarkButton({ x, label, href }: { x: number; label: string; href?: string }) {
  ...
  const text = (
    <span style={{ position: "absolute", left: 8, right: 8, top: 8, ...txt(12, 16, CREAM, "center"), fontWeight: 500 }}>
      {label}
    </span>
  );
  return href ? (
...
function LightButton({ x, label, href }: { x: number; label: string; href?: string }) {
  const style: React.CSSProperties = {
    ...abs(x, 392, 176, 32),
    background: CARD,
    boxShadow: `inset 0 0 0 1px ${SAND}`,
  ...
  const text = (
    <span style={{ position: "absolute", left: 8, right: 8, top: 8, ...txt(12, 16, INK, "center"), fontWeight: 500 }}>
      {label}
    </span>
  );
  return href ? (
...
function Dashboard({
  ...
  const tiles: Tile[] = shopping
    ? [
        { icon: "916-141", ink: [24, 24], title: "My Orders", text: "View all orders", href: "/account/orders" },
        { icon: "916-145", ink: [22, 22], title: "Wishlist", text: "8 saved gifts" },
        { icon: "916-149", ink: [24, 17], title: "Custom Archive", text: "Saved designs" },
        { icon: "916-153", ink: [19, 19], title: "Addresses", text: "Manage delivery" },
      ]
    : [
        { icon: "916-141", ink: [24, 24], title: "Purchase Orders", text: "Approved orders" },
        { icon: "916-145", ink: [22, 22], title: "Purchase Requests", text: "Quotes & pricing" },
        { icon: "916-149", ink: [24, 17], title: "Sample Requests", text: "Samples & status" },
        { icon: "916-153", ink: [19, 19], title: "Business Team", text: "Dedicated support" },
      ];
  ...
  return (
    <ScaleFrame height={873} background={CREAM} fontClass={notoSC.className} navActive="Account">
      ...
      <div style={{ ...abs(28, 93, 48, 48), background: PINK, borderRadius: 24 }} />
      <div className={playfair.className} style={{ ...abs(28, 101, 48), ...txt(26, 30, INK, "center"), fontWeight: 500 }}>
        {initials}
      </div>
      <div className={playfair.className} style={{ ...abs(90, 94, 240), ...txt(20, 24, INK), fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis" }}>
        Hello, {name}
      </div>
      <div style={{ ...abs(90, 120, 240), ...txt(13, 16, INK) }}>
        {shopping ? "Welcome back to GoldRose" : "Manage procurement and partnerships"}
      </div>

      {/* account-type toggle */}
      <div style={{ ...abs(16, 164, 398, 42), background: CARD, boxShadow: `inset 0 0 0 1px ${SAND}`, borderRadius: 10 }} />
      <div style={{ ...abs(shopping ? 16 : 215, 164, 199, 42), background: PINK, borderRadius: 10 }} />
      {shopping ? (
        <>
          <div style={{ ...abs(24, 176, 183), ...txt(12, 16, INK, "center"), fontWeight: 500 }}>Gift Shopping</div>
          <Link href="/account/business" style={{ ...abs(215, 164, 199, 42), display: "block" }}>
            <span style={{ position: "absolute", left: 8, top: 12, width: 183, ...txt(12, 16, INK, "center"), fontWeight: 500 }}>
        ...
        <>
          <Link href="/account" style={{ ...abs(16, 164, 199, 42), display: "block" }}>
            <span style={{ position: "absolute", left: 8, top: 12, width: 183, ...txt(12, 16, INK, "center"), fontWeight: 500 }}>
              Gift Shopping
            </span>
          </Link>
          <div style={{ ...abs(223, 176, 183), ...txt(12, 16, INK, "center"), fontWeight: 500 }}>Business &amp; Partnerships</div>
        </>
      )}

      {/* recent order / recent purchase request */}
      <div style={card(16, 218, 398, 218)} />
      <div className={playfair.className} style={{ ...abs(30, 230, 260), ...txt(shopping ? 18 : 15, shopping ? 22 : 18, INK), fontWeight: 600 }}>
        {shopping ? "Recent Order" : "Recent Purchase Request"}
      </div>
      {shopping ? (
        <Link href="/account/orders" style={{ ...abs(316, 233, 84, 15), display: "block" }}>
          <span style={{ ...txt(12, 15, INK, "right"), fontWeight: 500, display: "block" }}>View all&nbsp;&nbsp;›</span>
        </Link>
      ) : (
        <div style={{ ...abs(316, 233, 84), ...txt(12, 15, INK, "right"), fontWeight: 500 }}>View all&nbsp;&nbsp;›</div>
      )}
      ...
      {shopping ? (
        order ? (
          <>
            <img
              ...
              width={122}
              height={118}
              style={{ ...abs(30, 266, 122, 118), borderRadius: 12, objectFit: "cover", display: "block" }}
            />
            <div
              style={{ ...abs(166, 266, 230, 36), ...txt(14, 18, INK), fontWeight: 700, whiteSpace: "normal", overflow: "hidden" }}
              data-live-text
            >
              {order.title}
            </div>
            <div style={{ ...abs(166, 306, 230), ...txt(13, 16, INK), overflow: "hidden", textOverflow: "ellipsis" }} data-live-text>
              {order.line2}
            </div>
            <div style={{ ...abs(166, 328, 230), ...txt(13, 16, GOLD), fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis" }} data-live-text>
              {order.status}
            </div>
            <div style={{ ...abs(166, 354, 150), ...txt(20, 22, INK), fontWeight: 700 }} data-live-text>
              {order.price}
            </div>
          ...
          <>
            <div style={{ ...abs(30, 286, 340), ...txt(14, 18, INK), fontWeight: 700 }}>No orders yet.</div>
            <div style={{ ...abs(30, 312, 340), ...txt(13, 16, INK) }}>Your first gold rose is waiting in the shop.</div>
        ...
        <>
          <div style={{ ...abs(30, 266, 260, 21), ...txt(17, 21, INK), fontWeight: 700 }}>RFQ #GRB-20260821</div>
          <div style={{ ...abs(30, 300, 330), ...txt(13, 16, INK) }}>Corporate Gift Customization&nbsp; ·&nbsp; 500 units</div>
          <div style={{ ...abs(30, 326, 330), ...txt(13, 16, GOLD), fontWeight: 500 }}>IN REVIEW&nbsp; ·&nbsp; Updated Aug 22, 2:30 PM</div>
          <div style={{ ...abs(30, 354, 350), ...txt(12, 16, INK), fontWeight: 700 }}>Contact: David Zhang&nbsp; ·&nbsp; Business Manager</div>
      ...
      {tiles.map((tile, i) => {
        ...
        const body = (
          <>
            ...
            <span style={{ position: "absolute", left: 8, top: 52, width: 78, ...txt(11, 14, INK, "center"), fontWeight: 700, whiteSpace: "normal" }}>{tile.title}</span>
            <span style={{ position: "absolute", left: 8, top: 77, width: 78, ...txt(9, 12, INK, "center") }}>{tile.text}</span>
          </>
        );
        return tile.href ? (
          <Link key={tile.title} href={tile.href} style={{ ...card(x, 448, 94, 118), borderRadius: 12, display: "block" }}>
            {body}
          </Link>
        ) : (
          <div key={tile.title} style={{ ...card(x, 448, 94, 118), borderRadius: 12 }}>
            {body}
          </div>
        );
      })}
      ...
      {rows.map((row, i) => {
        ...
        const body = (
          <>
            <span style={{ ...abs(16, 0, 230), ...txt(12, 16, INK), fontWeight: 500 }}>{row.title}</span>
            <span style={{ ...abs(248, 0, 132), ...txt(11, 16, INK, "right") }}>{row.value}</span>
          </>
        );
        return (
          <div key={row.title}>
            {row.href ? (
              <Link href={row.href} style={{ ...abs(16, y, 398, 30), display: "block" }}>
                {body}
              </Link>
            ) : (
              <div style={{ ...abs(16, y, 398, 30) }}>{body}</div>
            )}
            {i < 3 ? <div style={{ ...abs(30, [618, 659, 700][i], 370, 1), background: SAND }} /> : null}
          </div>
        );
      })}
