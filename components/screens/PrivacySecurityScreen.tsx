...
import {
  ...
  sCard,
  SettingsHeader,
  SettingsToggle, GoldRoseWordmark } from "@/components/screens/account-chrome";
...
export function PrivacySecurityScreen() {
  ...
  return (
    <ScaleFrame height={932} background={CREAM} fontClass={notoSC.className} nav={false}>
      ...
      <div style={{ ...abs(36, 144, 245), ...txt(12, 20, INK), whiteSpace: "pre-line" }}>
        {"Manage your personal details, preferences,\nand privacy controls."}
      </div>
      <img src="/veloria/screens/1523-1082.svg" alt="" width={76} height={76} style={{ ...abs(320, 135, 76, 76), display: "block" }} />

      {/* security card (1523:1086) */}
      <div style={sCard(16, 239, 398, 520)} />
      <img src="/veloria/screens/1523-1087.svg" alt="" width={22} height={22} style={{ ...abs(34, 253, 22, 22), display: "block" }} />
      ...
      <div style={{ ...abs(302, 283, 96, 34), background: CREAM, boxShadow: `inset 0 0 0 1px ${GOLD}`, borderRadius: 8 }}>
        <span style={{ position: "absolute", left: 0, right: 0, top: 10, ...txt(9, 16, GOLD, "center"), fontWeight: 500, letterSpacing: 1.2 }}>
          Change password
        </span>
      </div>
      <div style={{ ...abs(34, 331, 112), ...txt(10, 16, INK) }}>New password</div>
      <div style={sCard(164, 323, 234, 38, { bg: CREAM, r: 8, shadow: false })} />
      <div style={{ ...abs(176, 334, 180), ...txt(10, 16, INK) }}>Enter new password</div>
      <Glyph src="1523-1097" x={369} y={333} w={20} h={20} ink={[9, 9]} />
      <div style={{ ...abs(34, 377, 120), ...txt(10, 16, INK) }}>Confirm password</div>
      <div style={sCard(164, 369, 234, 38, { bg: CREAM, r: 8, shadow: false })} />
      <div style={{ ...abs(176, 380, 180), ...txt(10, 16, INK) }}>Confirm new password</div>
      <Glyph src="1523-1101" x={369} y={379} w={20} h={20} ink={[9, 9]} />
      <div style={{ ...abs(164, 409, 234), ...txt(8, 16, INK) }}>Use 8+ characters with letters, numbers, and symbols.</div>
      <div style={{ ...abs(34, 433, 364, 1), background: SAND }} />

      {/* two-step verification — flips visually only */}
      <div style={{ ...abs(34, 451, 220), ...txt(11, 20, INK) }}>Two-step verification</div>
      <div style={{ ...abs(34, 471, 250), ...txt(8, 16, INK) }}>Add an extra layer of security to your account.</div>
      <SettingsToggle x={336} y={449} w={42} on={twoStep} onFlip={() => setTwoStep((v) => !v)} label="Two-step verification" />
      <div style={{ ...abs(382, 453, 24), ...txt(9, 16, INK) }}>{twoStep ? "On" : "Off"}</div>

      {/* login activity — the mock's own data */}
      <div style={{ ...abs(34, 503, 200), ...txt(12, 20, INK) }}>Login activity</div>
      <div style={{ ...abs(34, 523, 250), ...txt(8, 16, INK) }}>Review recent devices and account access.</div>
      <Glyph src="1523-1112" x={34} y={550} w={20} h={20} ink={[8, 10]} />
      <div style={{ ...abs(60, 551, 180), ...txt(11, 20, INK) }}>Current session</div>
      {SESSION_ROWS.map(([label, value, gold], i) => (
        <div key={label}>
          <div style={{ ...abs(60, 575 + i * 22, 110), ...txt(9, 16, INK) }}>{label}</div>
          <div style={{ ...abs(251, 575 + i * 22, 145), ...txt(9, 16, gold ? GOLD : INK, "right") }}>{value}</div>
        </div>
      ))}
      <Glyph src="1523-1122" x={34} y={666} w={20} h={20} ink={[8, 8]} />
      <div style={{ ...abs(60, 667, 180), ...txt(11, 20, INK) }}>Recent activity</div>
      {RECENT_ROWS.map(([device, meta], i) => (
        <div key={device}>
          <div style={{ ...abs(36, 691 + i * 22, 190), ...txt(8, 16, INK) }}>{device}</div>
          <div style={{ ...abs(230, 691 + i * 22, 166), ...txt(8, 16, INK, "right") }}>{meta}</div>
        </div>
      ))}

      {/* Save (inert, 07-29 ink primary) / Cancel (back) */}
      <div style={sCard(16, 782, 398, 54, { bg: INK, r: 14, stroke: false })} />
      <div style={{ ...abs(16, 798, 398), ...txt(15, 20, CREAM, "center") }}>✓&nbsp;&nbsp;Save changes</div>
      <button
        type="button"
        onClick={() => router.back()}
        style={{ ...sCard(16, 857, 398, 48, { r: 12, shadow: false }), border: 0, padding: 0, cursor: "pointer" }}
      >
