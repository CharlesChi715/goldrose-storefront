...
const CASES: Array<{
...
}> = [
  {
    ...
    pos: { title: 31.5, titleW: 220, order: 61, delivered: 84, price: 140, tag: 88, btn: 125, btnH: 29, btnText: 130, dateX: 296, dateW: 96, date: 159 },
  },
  {
    img: "1523-3849",
    ...
    pos: { title: 24, titleW: 240, order: 58, delivered: 79, price: 136, tag: 78, btn: 123, btnH: 31, btnText: 128.5, dateX: 292, dateW: 102, date: 158 },
  },
];
...
const HELP: Array<{ x: number; glyph: string; ink: [number, number]; title: string; note: string; href?: string }> = [
  { x: 22, glyph: "1523-3862", ink: [21, 21], title: "Return Policy", note: "View eligibility" },
  { x: 152, glyph: "1523-3866", ink: [13, 13], title: "Refund Timeline", note: "How it works" },
  { x: 282, glyph: "1523-3870", ink: [19, 19], title: "Contact Support", note: "We’re here to help", href: "/care/chat" },
];
...
export function ReturnsScreen() {
  ...
  return (
    <ScaleFrame height={932} background={CREAM} fontClass={notoSC.className} nav={false}>
      ...
      <BackButton fallback="/account" src="/veloria/screens/1523-3875.png" style={abs(13, 24.5, 40, 43)} />
      <GoldRoseWordmark x={143} y={20.5} w={140} h={51} />
      <Link href="/checkout" aria-label="Bag" style={{ ...abs(373, 24.5, 40, 43), display: "block" }}>
        <img src="/veloria/screens/1523-3877.png" alt="" width={40} height={43} style={{ display: "block" }} />
      </Link>

      {/* 1523:3827/3828 title + subtitle */}
      <div className={playfair.className} style={{ ...abs(42, 92, 342), ...txt(27, 40, INK, "center"), fontWeight: 500, letterSpacing: -0.2 }}>
        Returns &amp; After-Sales
      </div>
      <div style={{ ...abs(34, 134, 358), ...txt(11, 20, INK, "center"), whiteSpace: "pre-line" }}>
      ...
      {(
        ...
        <button
          ...
          onClick={() => setTab(key)}
          style={{ ...abs(x, 188, 199, 50), border: 0, padding: 0, background: "transparent", cursor: "pointer" }}
        >
          <span style={{ position: "absolute", left: 0, right: 0, top: 16, ...txt(13, 20, INK, "center") }}>{label}</span>
          {tab === key ? <span style={{ ...abs(39, 41, 120, 2), background: GOLD, display: "block" }} /> : null}
        </button>
      ))}
      ...
      <div style={{ ...abs(71, 268, 230), ...txt(14, 20, CREAM) }}>Current Orders Only</div>
      <div style={{ ...abs(71, 292, 290), ...txt(9, 16, CREAM), whiteSpace: "pre-line" }}>
        ...
        return (
          <div key={item.order}>
            ...
            <img
              ...
              width={116}
              height={144}
              style={{ ...abs(26, y + 16, 116, 144), borderRadius: 10, display: "block" }}
            />
            <div className={goudy.className} style={{ ...abs(158, y + p.title, p.titleW), ...txt(14, 20, INK), fontWeight: 500 }}>
              {item.title}
            </div>
            <div style={{ ...abs(158, y + p.order, 220), ...txt(9, 16, INK) }}>{item.order}</div>
            <div style={{ ...abs(158, y + p.delivered, 220), ...txt(9, 16, GOLD) }}>{item.delivered}</div>
            <div style={{ ...abs(158, y + p.price, 100), ...txt(15, 20, INK) }}>{item.price}</div>
            <div style={{ ...abs(292, y + p.tag, 102, 32), background: INK, borderRadius: 8 }}>
              <span style={{ position: "absolute", left: 0, right: 0, top: 6, ...txt(10, 16, CREAM, "center") }}>{item.status}</span>
            </div>
            <div style={sCard(292, y + p.btn, 102, p.btnH, { bg: CREAM, r: 8, shadow: false })} />
            <div style={{ ...abs(292, y + p.btnText, 102), ...txt(10, 20, INK, "center") }}>{item.action}</div>
            <div style={{ ...abs(p.dateX, y + p.date, p.dateW), ...txt(8, 16, INK, "center") }}>{item.date}</div>
          </div>
        );
      })}
      ...
      <div style={{ ...abs(14, 743, 398), ...txt(15, 20, INK, "center") }}>After-Sales Help</div>
      {HELP.map((item) => {
        const body = (
          <>
        ...
        return item.href ? (
          <Link key={item.title} href={item.href} style={{ ...abs(item.x, 775, 126, 92), display: "block" }}>
            {body}
          </Link>
        ) : (
      ...
      <div style={{ ...abs(30, 894, 366), ...txt(9, 16, INK, "center") }}>Need more help? Our support team is available 24/7.</div>
    </ScaleFrame>
  );
}
