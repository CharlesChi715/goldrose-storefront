 ...
 * travels in the link's `utm_acc` tag — our own param, not one of the five
 * standard UTM params, so ad tools can never auto-fill it with an ad-variant
 * value and corrupt the commission report (decision 2026-07-24, see
 * docs/features/posting-account-attribution.md; deliberately no fallback to
 * `utm_content`). The label is prefixed with the channel so same-named
 * accounts on different platforms stay separate.
 *
