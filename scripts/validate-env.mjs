...
// once real money is switched on — so pairing it with a LIVE Stripe key (the
...
// whether money is real: sk_live_, or rk_live_ for a restricted key.
//
// The webhook is the safety net under the browser-driven return leg: it is
// what repairs an order when the buyer's browser dies between paying and our
// response. Without STRIPE_WEBHOOK_SECRET the signature check cannot pass, so
...
// that is protected. Loud whenever Stripe is configured at all, fatal when the
// money is real.
const stripeKey = (process.env.STRIPE_SECRET_KEY ?? "").trim();
const stripeLive = /^(sk|rk)_live_/.test(stripeKey);
