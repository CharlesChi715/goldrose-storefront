...
test("personal info renders as an inert placeholder (no live inputs)", async ({ page }) => {
...
test("security ships placeholder password fields, a live Cancel, an inert Save", async ({ page }) => {
...
test("delete account stays inert: no live input, no live delete button", async ({ page }) => {
  ...
  const box = page.getByRole("checkbox", { name: "I understand the consequences" });
  await box.click();
  await expect(box).toHaveAttribute("aria-checked", "true");
});

test("returns renders the mock cases and Contact Support reaches the chat", async ({ page }) => {
...
test("care's Chat with us reaches the support chat mock", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/care\/chat$/);
  await expect(page.getByText("GoldRose Support", { exact: true }).first()).toBeVisible();
