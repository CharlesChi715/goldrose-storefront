...
/** The stage is 5015px of bands plus the 59px bottom nav. */
const NAV_HEIGHT = 59;
...
test("hiding one band slides only the bands below it, by its height", () => {
  ...
  assert.equal(shift.occasion, -327);
  assert.equal(shift.recipient, -327);
  assert.equal(shift.craft, -327);
  assert.equal(shift.story, -327);
});
